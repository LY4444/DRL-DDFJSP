...
TM = 3  # 同一工厂内转运时间
TF = 6  # 不同工厂间转运时间
U = {1: 1.3, 2: 1.2}  # 工件权重（与原OPL一致）
M = 100000  # 大M参数

# -------------------------- 数据矩阵定义（严格对应你提供的CPLEX数据）--------------------------
# 维度说明：3个工厂 × 2台机器 × 2个工件 × 3道工序（车间→机器→工件→工序）

# 1. 可选机器矩阵 x_fkij[工厂f][机器k][工件i][工序j]（0/1，完全复用你的CPLEX数据）
x_cplex = [
    [
        [[0, 1, 0], [1, 0, 1]],  # 工厂1-机器1（原M1）
        [[1, 0, 1], [0, 1, 0]]  # 工厂1-机器2（原M2）
    ],
    [
        [[0, 1, 0], [1, 0, 1]],  # 工厂2-机器1（原M3）
        [[1, 0, 1], [0, 1, 0]]  # 工厂2-机器2（原M4）
    ],
    [
        [[0, 1, 0], [1, 0, 1]],  # 工厂3-机器1（原M5）
        [[1, 0, 1], [0, 1, 0]]  # 工厂3-机器2（原M6）
    ]
]
...
pt_cplex = [
    [
        [[71, 64, 72], [61, 35, 44]],  # 工厂1-机器1（原M1）
        [[74, 36, 25], [20, 58, 19]]  # 工厂1-机器2（原M2）
    ],
    [
        [[48, 66, 4], [45, 4, 55]],  # 工厂2-机器1（原M3）
        [[33, 15, 20], [43, 7, 18]]  # 工厂2-机器2（原M4）
    ],
    [
        [[34, 18, 70], [49, 6, 41]],  # 工厂3-机器1（原M5）
        [[8, 48, 34], [52, 45, 33]]  # 工厂3-机器2（原M6）
    ]
]
...
bd_cplex = [
    [
        [[0, 1, 0], [1, 0, 1]],  # 工厂1-机器1（原M1）
        [[0, 0, 1], [0, 1, 0]]  # 工厂1-机器2（原M2）
    ],
    [
        [[1, 1, 1], [1, 1, 0]],  # 工厂2-机器1（原M3）
        [[1, 1, 0], [0, 1, 1]]  # 工厂2-机器2（原M4）
    ],
    [
        [[0, 0, 1], [0, 0, 0]],  # 工厂3-机器1（原M5）
        [[0, 0, 1], [0, 0, 1]]  # 工厂3-机器2（原M6）
    ]
]
...
rt_cplex = [
    [
        ...
        [[2.1, 2.1, 2.1], [2.1, 2.1, 2.1]]  # 工厂1-机器2（原M2）
    ],
    [
        [[3.4, 3.4, 3.4], [3.4, 3.4, 3.4]],  # 工厂2-机器1（原M3）
        [[3.6, 3.6, 3.6], [3.6, 3.6, 3.6]]  # 工厂2-机器2（原M4）
    ],
    [
        [[3.1, 3.1, 3.1], [3.1, 3.1, 3.1]],  # 工厂3-机器1（原M5）
        [[2.7, 2.7, 2.7], [2.7, 2.7, 2.7]]  # 工厂3-机器2（原M6）
    ]
]
...
# -------------------------- 创建Gurobi模型 --------------------------
model = gp.Model("JobShop_Scheduling_with_Fault_2Jobs_3Factories")
            ...
            for f in F:
                for k in K_ijf:
                    if X[i, j, f, k].x > 0.5:  # 找到分配的机器
                        ...
                        original_machine = 2 * (f - 1) + k  # 还原原CPLEX机器号（M1-M6：3工厂×2机器）
                        print(
