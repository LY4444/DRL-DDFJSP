...
r_f = 2  # 工厂（车间）数量（原OPL r_f=2）
F = range(1, r_f + 1)  # 工厂索引 1..2
m_ijf = 3  # 每个工厂中能处理O_ij的机器数量（原OPL m_ijf=3）
...
TM = 3  # 同一工厂内转运时间
TF = 6  # 不同工厂间转运时间
U = {1: 1.5, 2: 1.2, 3: 1.0}  # 工件权重（原与原OPL一致）
M = 100000  # 大M参数

# -------------------------- 数据矩阵定义（严格对应CPLEX数据）--------------------------
# 维度说明：2个工厂 × 3台机器 × 3个工件 × 3道工序（车间→机器→工件→工序）

# 1. 可选机器矩阵 x_fkij[工厂f][机器k][工件i][工序j]（0/1，完全复用CPLEX数据）
x_cplex = [
    [
        [[1, 0, 0], [0, 1, 0], [0, 0, 1]],  # 工厂1-机器1（原M1）
        [[0, 1, 0], [0, 0, 1], [1, 0, 0]],  # 工厂1-机器2（原M2）
        [[0, 0, 1], [1, 0, 0], [0, 1, 0]]  # 工厂1-机器3（原M3）
    ],
    [
        [[1, 0, 0], [0, 1, 0], [0, 0, 1]],  # 工厂2-机器1（原M4）
        [[0, 1, 0], [0, 0, 1], [1, 0, 0]],  # 工厂2-机器2（原M5）
        [[0, 0, 1], [1, 0, 0], [0, 1, 0]]  # 工厂2-机器3（原M6）
    ]
]

# 2. 加工时间矩阵 pt_fkij[工厂f][机器k][工件i][工序j]（完全复用CPLEX数据）
pt_cplex = [
    [
        [[47, 47, 20], [36, 18, 35], [1, 8, 38]],  # 工厂1-机器1（原M1）
        [[74, 36, 25], [69, 8, 2], [20, 58, 19]],  # 工厂1-机器2（原M2）
        [[13, 37, 68], [63, 22, 59], [9, 26, 23]]  # 工厂1-机器3（原M3）
    ],
    [
        [[11, 18, 4], [30, 73, 49], [33, 30, 64]],  # 工厂2-机器1（原M4）
        [[48, 66, 4], [75, 62, 35], [45, 4, 55]],  # 工厂2-机器2（原M5）
        [[13, 11, 48], [71, 64, 72], [61, 35, 44]]  # 工厂2-机器3（原M6）
    ]
]

# 3. 故障发生率矩阵 bd_fkij[工厂f][机器k][工件i][工序j]（完全复用CPLEX数据）
bd_cplex = [
    [
        [[0, 0, 1], [0, 1, 0], [1, 1, 1]],  # 工厂1-机器1（原M1）
        [[1, 0, 0], [0, 0, 1], [1, 0, 0]],  # 工厂1-机器2（原M2）
        [[1, 0, 1], [0, 0, 1], [0, 0, 1]]  # 工厂1-机器3（原M3）
    ],
    [
        [[0, 1, 1], [1, 0, 0], [0, 0, 1]],  # 工厂2-机器1（原M4）
        [[0, 0, 1], [1, 1, 1], [0, 1, 1]],  # 工厂2-机器2（原M5）
        [[1, 1, 0], [1, 1, 1], [0, 0, 1]]  # 工厂2-机器3（原M6）
    ]
]
...
rt_cplex = [
    [
        [[2.8, 2.8, 2.8], [2.8, 2.8, 2.8], [2.8, 2.8, 2.8]],  # 工厂1-机器1（原M1）
        [[3.7, 3.7, 3.7], [3.7, 3.7, 3.7], [3.7, 3.7, 3.7]],  # 工厂1-机器2（原M2）
        [[2.7, 2.7, 2.7], [2.7, 2.7, 2.7], [2.7, 2.7, 2.7]]  # 工厂1-机器3（原M3）
    ],
    [
        [[3.4, 3.4, 3.4], [3.4, 3.4, 3.4], [3.4, 3.4, 3.4]],  # 工厂2-机器1（原M4）
        [[2.6, 2.6, 2.6], [2.6, 2.6, 2.6], [2.6, 2.6, 2.6]],  # 工厂2-机器2（原M5）
        [[2.5, 2.5, 2.5], [2.5, 2.5, 2.5], [2.5, 2.5, 2.5]]  # 工厂2-机器3（原M6）
    ]
]
...
# -------------------------- 创建Gurobi模型 --------------------------
model = gp.Model("JobShop_Scheduling_with_Fault_3Jobs_2Factories")
            ...
            for f in F:
                for k in K_ijf:
                    if X[i, j, f, k].x > 0.5:  # 找到分配的机器
                        ...
                        original_machine = 3 * (f - 1) + k  # 还原原CPLEX机器号（M1-M6：2工厂×3机器）
                        print(
