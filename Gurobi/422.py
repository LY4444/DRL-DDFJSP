...
# -------------------------- 参数定义（与原OPL完全一致）--------------------------
n = 2  # 工件数量（原OPL n=2）
I = range(1, n + 1)  # 工件索引 1..2
n_i = 3  # 每个工件的工序数量（原OPL n_i=3）
J_i = range(1, n_i + 1)  # 工序索引 1..3
r_f = 4  # 工厂（车间）数量（原OPL r_f=4）
F = range(1, r_f + 1)  # 工厂索引 1..4
m_ijf = 2  # 每个工厂中能处理O_ij的机器数量（原OPL为2台）
K_ijf = range(1, m_ijf + 1)  # 机器索引 1..2

# 基础参数（完全沿用原OPL定义）
AT = {1: 2, 2: 3}  # 工件i的到达时间
DD = {1: 10, 2: 12}  # 工件i的交货期
TM = 3  # 同一工厂内转运时间
TF = 6  # 不同工厂间转运时间
U = {1: 1.0, 2: 1.4}  # 工件权重（与原OPL一致）
M = 100000  # 大M参数

# -------------------------- 数据矩阵定义（严格对应你提供的CPLEX数据）--------------------------
# 维度说明：4个工厂 × 2台机器 × 2个工件 × 3道工序（车间→机器→工件→工序）

# 1. 可选机器矩阵 x_fkij[工厂f][机器k][工件i][工序j]（0/1，完全复用你的CPLEX数据）
x_cplex = [
    [
        [[1, 0, 1], [0, 1, 0]],  # 工厂1-机器1（M1）
        [[0, 1, 0], [1, 0, 1]]  # 工厂1-机器2（M2）
    ],
    [
        [[1, 0, 1], [0, 1, 0]],  # 工厂2-机器1（M3）
        [[0, 1, 0], [1, 0, 1]]  # 工厂2-机器2（M4）
    ],
    [
        [[1, 0, 1], [0, 1, 0]],  # 工厂3-机器1（M5）
        [[0, 1, 0], [1, 0, 1]]  # 工厂3-机器2（M6）
    ],
    [
        [[1, 0, 1], [0, 1, 0]],  # 工厂4-机器1（M7）
        [[0, 1, 0], [1, 0, 1]]  # 工厂4-机器2（M8）
    ]
]
...
pt_cplex = [
    [
        [[74, 36, 25], [20, 58, 19]],  # 工厂1-机器1（M1）
        [[63, 22, 59], [9, 26, 23]]  # 工厂1-机器2（M2）
    ],
    [
        [[11, 18, 4], [30, 73, 49]],  # 工厂2-机器1（M3）
        [[56, 55, 50], [61, 7, 41]]  # 工厂2-机器2（M4）
    ],
    [
        [[7, 31, 23], [49, 6, 41]],  # 工厂3-机器1（M5）
        [[47, 47, 20], [36, 18, 35]]  # 工厂3-机器2（M6）
    ],
    [
        [[59, 40, 10], [33, 15, 20]],  # 工厂4-机器1（M7）
        [[71, 64, 72], [61, 35, 44]]  # 工厂4-机器2（M8）
    ]
]
...
bd_cplex = [
    [
        [[0, 1, 0], [0, 1, 1]],  # 工厂1-机器1（M1）
        [[0, 0, 1], [0, 1, 0]]  # 工厂1-机器2（M2）
    ],
    [
        [[1, 1, 0], [1, 0, 1]],  # 工厂2-机器1（M3）
        [[0, 1, 1], [1, 0, 1]]  # 工厂2-机器2（M4）
    ],
    [
        [[0, 0, 1], [0, 0, 0]],  # 工厂3-机器1（M5）
        [[1, 0, 0], [0, 0, 1]]  # 工厂3-机器2（M6）
    ],
    [
        [[1, 0, 0], [0, 0, 1]],  # 工厂4-机器1（M7）
        [[0, 0, 1], [0, 1, 1]]  # 工厂4-机器2（M8）
    ]
]

# 4. 故障维修时间矩阵 rt_fkij[工厂f][机器k][工件i][工序j]（完全复用你的CPLEX数据）
rt_cplex = [
    [
        [[3.6, 3.6, 3.6], [3.6, 3.6, 3.6]],  # 工厂1-机器1（M1）
        [[2.1, 2.1, 2.1], [2.1, 2.1, 2.1]]  # 工厂1-机器2（M2）
    ],
    [
        [[3.8, 3.8, 3.8], [3.8, 3.8, 3.8]],  # 工厂2-机器1（M3）
        [[2.6, 2.6, 2.6], [2.6, 2.6, 2.6]]  # 工厂2-机器2（M4）
    ],
    [
        [[3.1, 3.1, 3.1], [3.1, 3.1, 3.1]],  # 工厂3-机器1（M5）
        [[2.7, 2.7, 2.7], [2.7, 2.7, 2.7]]  # 工厂3-机器2（M6）
    ],
    [
        [[2.3, 2.3, 2.3], [2.3, 2.3, 2.3]],  # 工厂4-机器1（M7）
        [[2.5, 2.5, 2.5], [2.5, 2.5, 2.5]]  # 工厂4-机器2（M8）
    ]
]
...
# -------------------------- 创建Gurobi模型 --------------------------
model = gp.Model("JobShop_Scheduling_with_Fault_ActualData")

# -------------------------- 决策变量定义（与原OPL一一对应）--------------------------
X = model.addVars(I, J_i, F, K_ijf, vtype=GRB.BINARY, name="X")  # 机器分配
Y = model.addVars(I, J_i, I, J_i, vtype=GRB.BINARY, name="Y")  # 工序顺序
Z1 = model.addVars(I, J_i, vtype=GRB.BINARY, name="Z1")  # 厂内转运
Z2 = model.addVars(I, J_i, vtype=GRB.BINARY, name="Z2")  # 厂间转运
...
MTJ = model.addVar(vtype=GRB.CONTINUOUS, name="MTJ")  # 平均延期时间（目标）
            ...
            for f in F:
                for k in K_ijf:
                    if X[i, j, f, k].x > 0.5:  # 找到分配的机器
                        ...
                        print(
                            f"    工序{j}: 工厂{f}机器{k}（原M{2 * (f - 1) + k}） | 开始时间: {start_time:.2f} | 加工时间: {process_time} | 故障时间: {fault_time:.1f} | {transfer_type}")
