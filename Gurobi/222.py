...
m_ijf = 2  # 每个工厂中能处理O_ij的机器数量（原OPL m_ijf=2）
...
TM = 3  # 同一工厂内转运时间
TF = 6  # 不同工厂间转运时间
U = {1: 1.3, 2: 1.5}  # 工件权重（与原OPL一致）
M = 100000  # 大M参数

# -------------------------- 数据矩阵定义（严格对应CPLEX数据）--------------------------
# 维度说明：2个工厂 × 2台机器 × 2个工件 × 3道工序（车间→机器→工件→工序）

# 1. 可选机器矩阵 x_fkij[工厂f][机器k][工件i][工序j]（0/1，完全复用CPLEX数据）
x_cplex = [
    [
        [[1, 0, 1], [0, 1, 0]],  # 工厂1-机器1（原M1）
        [[0, 1, 0], [1, 0, 1]]  # 工厂1-机器2（原M2）
    ],
    [
        [[1, 0, 1], [0, 1, 0]],  # 工厂2-机器1（原M3）
        [[0, 1, 0], [1, 0, 1]]  # 工厂2-机器2（原M4）
    ]
]
...
pt_cplex = [
    [
        [[74, 36, 25], [20, 58, 19]],  # 工厂1-机器1（原M1）
        [[27, 8, 68], [37, 46, 14]]  # 工厂1-机器2（原M2）
    ],
    [
        [[11, 18, 4], [30, 73, 49]],  # 工厂2-机器1（原M3）
        [[13, 11, 48], [61, 35, 44]]  # 工厂2-机器2（原M4）
    ]
]
...
bd_cplex = [
    [
        [[0, 0, 1], [0, 1, 1]],  # 工厂1-机器1（原M1）
        [[0, 0, 1], [1, 1, 1]]  # 工厂1-机器2（原M2）
    ],
    [
        [[0, 1, 0], [1, 0, 1]],  # 工厂2-机器1（原M3）
        [[0, 1, 1], [1, 0, 1]]  # 工厂2-机器2（原M4）
    ]
]
...
rt_cplex = [
    [
        [[3.6, 3.6, 3.6], [3.6, 3.6, 3.6]],  # 工厂1-机器1（原M1）
        [[2.3, 2.3, 2.3], [2.3, 2.3, 2.3]]  # 工厂1-机器2（原M2）
    ],
    [
        [[3.8, 3.8, 3.8], [3.8, 3.8, 3.8]],  # 工厂2-机器1（原M3）
        [[3.4, 3.4, 3.4], [3.4, 3.4, 3.4]]  # 工厂2-机器2（原M4）
    ]
]
...
# -------------------------- 创建Gurobi模型 --------------------------
model = gp.Model("JobShop_Scheduling_with_Fault_2Jobs_2Factories_2Machines")
            ...
            for f in F:
                for k in K_ijf:
                    if X[i, j, f, k].x > 0.5:  # 找到分配的机器
                        ...
                        original_machine = 2 * (f - 1) + k  # 还原原CPLEX机器号（M1-M4：2工厂×2机器）
                        print(
